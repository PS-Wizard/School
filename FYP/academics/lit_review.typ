...
+ If $F(p_i) = +1$ (opponent wins $p_i$), then $-F(p_i) = -1$ (losing for current)
+ If $F(p_i) = -1$ (opponent loses $p_i$), then $-F(p_i) = +1$ (winning for current)
...
== Pruning
A game of chess typically lasts ~40 moves, and with a branching factor of 35, at that number there are $~10^24$ possible positions reachable from the starting position. As such, it is unfeasible to do an exhaustive search. The time complexity with just negamax is $O(b^d)$, where $b = "branching factor (~35 in chess)", d = "depth"$ @shannon_1950_programming[p. 4] @marsland[p. 4]. This computational reality has driven the development of pruning techniques that maintain search accuracy while dramatically reducing the nodes examined.

=== Branch-and-Bound Optimization
Knuth and Moore first present an optimization that improves upon the pure negamax function (say, $F$) as $F_1$. $F_1$ improves $F$ by introducing an upper bound to prune moves that can't be better than the already known options. Knuth and Moore define:
$
  F_1(p, "bound") = cases(
    F(p) "if" F(p) < "bound",
    >= "bound" "if" F(p) >= "bound"
...
$
@knuth_1975_an[p.5]. The intuition behind $F_1$ is that when evaluating a position $p$ from the current player's perspective with a known bound that represents the best value achievable till now, $F_1$ computes and returns the value if it is less than the bound, or "$>= "bound"$" if it is equal or greater than the bound; i.e., once it determines a move that achieves a value at least as good or better than our current best option, it prunes away the branch.
...
@knuth_1975_an[p.6]. Pruning happens when $alpha >= beta$, the intuition behind which is that the maximizing player already has an option $alpha$ that is at least as good as what the opponent will allow $beta$. Thus, the minimizing player won't allow reaching this position, because we assume optimal play, so we prune that branch @marsland[p.4].

==== Deep Cutoffs
A significant advantage of alpha-beta over the single bounded approach is its ability to do "deep cutoffs". Knuth and Moore demonstrated that $F_2(-infinity, +infinity)$ examines the same number of nodes as $F_1(p,infinity)$ until the fourth look-ahead level, but on the fourth and beyond levels, $F_2$ occasionally makes deep cutoffs that $F_1$ isn't capable of finding @knuth_1975_an[p.2, p.7]. This capability represents a fundamental improvement in pruning efficiency that has made alpha-beta the foundation for all subsequent search optimizations @knuth_1975_an[p.6].
...
=== Critical Perspective on Alpha-Beta Dominance
While Knuth and Moore's proof established alpha-beta's theoretical optimality within the framework of brute-force minimax search, recent developments challenge whether this paradigm itself remains optimal. AlphaZero's success with Monte Carlo Tree Search (MCTS) guided by neural networks demonstrated that selective search based on learned policy estimates can outperform traditional alpha-beta approaches, despite examining approximately 1000x fewer positions @mastering[p.3-p.5]. This represents a fundamental paradigm shift: from guaranteed pruning through mathematical bounds (the alpha-beta approach) to probabilistic selection through learned heuristics (the MCTS/NN approach).

Traditional alpha-beta engines seek brute-force speed through guaranteed pruning, while neural network-guided MCTS engines seek efficiency through highly accurate selectivity learned from self-play @mastering[p.3-p.5]. While earlier MCTS implementations proved weaker than alpha-beta @mastering[p.12], coupling MCTS with deep neural networks achieved superiority, challenging the widespread belief that alpha-beta was inherently better suited for chess. However, the subsequent integration of NNUE into Stockfish suggests a convergence rather than replacement, with modern engines combining alpha-beta's efficiency with neural evaluation @Stockfish2025NNUEWiki. A core research gap remains in definitively quantifying the performance comparison between these evolved approaches under varying time controls and hardware constraints.

== The Horizon Effect
Shannon was amongst the earliest to recognize what is known today as the "horizon effect" @shannon_1950_programming[p.6]. This effect describes a program's tendency to "hide" the inevitable material loss by making delaying moves until said loss is far enough out of its maximum depth (the horizon). This problem emerges from a lack of computing power that forces programs to limit the depth of the search and make the "best move" based on incomplete information @Brange[p.14].
...
Shannon acknowledged the importance of evaluating only those positions that are "relatively quiescent" @shannon_1950_programming[p.6]. A quiescent position is one that can be assessed accurately without needing further deepening @marsland[p.7]. This matters because positions at the horizon frequently occur with tactical sequences like captures, checks, or other forcing moves, creating a situation that defies accurate static evaluation @marsland[p.19].

=== Quiescence Search
Quiescence search is the principal approach to solving the horizon effect problem, by ensuring that a position is stable before evaluation. Sources universally agree that Quiescence Search (QS) is critical for handling tactical volatility, extending search beyond the depth limit (horizon) to ensure evaluation occurs only in "quiet" (non-forcing) positions @marsland[p.7] @parallel_chess_searching[p.41] @bijl_2021_exploring[p.11]. It is a type of search extension that continues evaluation of all the forcing moves until a "quiet" position is reached. Rather than terminating the search at a fixed depth regardless of the position's characteristics, quiescence search adapts, extending analysis in tactical positions.

However, a consistent theoretical framework defining true quiescence remains undeveloped, forcing reliance on heuristic thresholds for when QS should stop @marsland[p.8]. This represents an ongoing research gap: what exactly constitutes a "quiet" position varies between implementations, and no rigorous mathematical definition has emerged to replace the intuitive heuristics currently employed.
...
This is arguably the most intuitive representation as it directly reflects a normal chess board. Despite its intuitiveness, this approach comes with performance costs. Indexing the array requires calculating the memory location `8 * rank + file` and performing multiple boundary checks, making it inefficient @bijl_2021_exploring[p.4] @tessaract[p.6]. Bijl & Tiet's testing showed that this 2D array approach was the slowest, coming in at 39.189mnps in Perft and 6.327mnps in search speed @bijl_2021_exploring[p.19].
...
=== Critical Perspective on Representation Choice
While sources universally agree that bitboards are the preferred representation for competitive engines @alphadeepchess[p.30] @bijl_2021_exploring[p.5], the reasons why reveal an important nuance. Bijl & Tiet's empirical findings challenge the conventional narrative: traditional array-based representations like 0x88 boards can achieve comparable speeds to bitboards in basic move generation (Perft tests) @bijl_2021_exploring[p.20]. However, bitboards are preferred for full-purpose engines because they accelerate subsequent operations like evaluation, which heavily relies on fast bitwise checks, where array-based methods falter @tessaract[p.10] @bijl_2021_exploring[p.20].

This distinction is crucial: the observation that board architecture performance is mostly only relevant in move generation is false. While move generators perform adequately with both architectures, evaluation speeds benefit significantly from bitboard-style engines @bijl_2021_exploring[p.20]. In Bijl & Tiet's complete engine tests, move generation accounted for only about 10% of processing time, with evaluation forming the primary bottleneck. As such, bitboards remain the optimal choice not solely because of move generation speed, but because of their overall performance benefits across the entire engine pipeline.
...
==== PEXT Boards
The PEXT instruction, part of the BMI2 instruction set, acts as an alternative to magic bitboards and multiplicative hashing. The later availability of the PEXT (Parallel Bits Extract) CPU instruction (on Intel Haswell and newer processors, introduced in 2013) made PEXT Bitboards the new theoretical state-of-the-art for performance, as PEXT directly computes the necessary index in a single CPU cycle, eliminating the need for magic numbers @hash_funtions[p.10] @alphadeepchess[p.51] @bijl_2021_exploring[p.8] @tessaract[p.10]. The PEXT instruction performs parallel bit extraction in a single CPU cycle:
...
==== The Magic vs. PEXT Debate
The conflict between Magic Bitboards and PEXT Bitboards represents a hardware-driven evolution rather than a purely algorithmic advancement. The divergence is due to hardware evolution: PEXT's superiority is entirely contingent on the availability of this specific instruction set, making Magic Bitboards the essential fallback option for platforms lacking this instruction @hash_funtions[p.10]. However, empirical testing reveals a surprising result. Based on 100 runs of Stockfish's benchmark suite, PEXT bitboards provide only a 2.3% speedup over Magic Bitboards @hash_funtions[p.10].

Bijl & Tiet's findings corroborate this minimal practical difference. Although PEXT boards should theoretically be faster, it is likely such an insignificant increase on modern processors that it is hard to see a difference in the results @bijl_2021_exploring[p.20]. This reveals an important lesson about chess engine optimization: theoretical improvements don't always translate to meaningful practical gains. The choice between Magic and PEXT thus becomes less about raw performance and more about implementation tradeoffs,PEXT offers simpler code without magic number generation, while Magic provides broader hardware compatibility.

However, it's worth noting that older chipsets (machines running pre-Haswell for Intel and some pre-Excavator/pre-Zen for AMD) don't support BMI2, making it mandatory to use magic bitboards if support for these systems is desired @tessaract[p.10]. A research gap remains in definitively assessing the long-term, practical overhead imposed by bitboard representation in complex parallel search architectures, such as comparing the cost of transferring large bitboard data between threads versus recalculating the data locally @parallel_chess_searching[p.87].
...
==== Performance
When Tesseract switched to this 16 bit encoding scheme from a naive struct/classes implementation, it reported an almost 50% increase in move generation speed, which is quite significant @tessaract[p.12].
...
=== The Limitations of Hand-Crafted Evaluation

Despite their historical dominance and continued utility, HCE functions face a fundamental limitation: they rely on human domain expertise but are bounded by the complexity humans can explicitly model @mastering[p.2] @shannon_1950_programming[p.5]. Traditional evaluation is inherently designed on human metrics and heuristics, meaning even the best HCE is theoretically limited to the level of the best human players' explicit understanding. This constraint becomes particularly evident in complex positional relationships that evade human explicit definition,situations where intuition exists but formalization proves elusive. This limitation set the stage for the paradigm shift toward neural network-based evaluation, which we examine in subsequent sections.
...
#par(first-line-indent: 0em)[
  The effectiveness of LMR is heavily dependent on move ordering quality, revealing a critical conflict in the literature. LMR is implemented in top engines like Stockfish, indicating its value when refined @bijl_2021_exploring[p.12]. However, several developers found implementing LMR reliably resulted in worse performance or Elo drops due to its over-aggressive pruning settings causing blunders @alphadeepchess[p.63] @tessaract[p.27]. AlphaDeepChess reported no improvement from implementing LMR due to insufficient move ordering strength @alphadeepchess[p.65]. In contrast, the Tesseract engine demonstrated significant performance gains, reducing average search time from 83.87 to 64.13 milliseconds, though with a corresponding decrease in score from 8584 to 8124 @tessaract[p.26].
]

#par(first-line-indent: 0em)[
  This divergence is explained by testing prerequisites: LMR is a sensitive technique whose net benefit is heavily dependent on the quality of auxiliary systems, especially accurate move ordering heuristics that prevent good moves from being misclassified as "late". Weak supporting systems cause LMR to fail @alphadeepchess[p.63] @tessaract[p.27]. This trade-off illustrates the delicate nature of LMR and other aggressive pruning techniques. A research gap remains in creating accessible and robust implementations of aggressive pruning like LMR that do not suffer debilitating search instability or blunders when used outside of the highly optimized environments of top commercial engines @tessaract[p.26].
]
...
=== Critical Perspective on Speculative Pruning
#par(first-line-indent: 0em)[
  Sources conflict on the general reliability of "forward pruning." Forward pruning that aggressively discards branches based only on weak heuristics (like "tapered N-best search") is highly dangerous and unreliable @marsland[p.3]. However, pruning supported by reliable external data, such as TPT entries confirming a subtree is fully solved, constitutes "exact forward pruning" and is beneficial @marsland[p.13]. The conflict is one of algorithmic rigor: pruning based on guesswork is bad; pruning based on established truth (e.g., hash hits) is good. This distinction separates successful techniques like NMP (with proper constraints) from failed speculative approaches.
]
...
#par(first-line-indent: 0em)[
  HCEs, despite their historical value, have objective drawbacks. The main drawback of HCE is that it is inherently designed on human metrics and heuristics. This limitation means that even the best HCE is limited to the same level as the best human player. This traditional approach is now being challenged or replaced by Neural Networks (NN), which capture complex, non-linear positional relationships that evade human explicit definition @mastering[p.12] @Nasu2018NNUE[p.1]. Traditional evaluation relies on human domain expertise but is limited by the complexity humans can model. The neural network approach leverages machine learning to overcome these limitations, resulting in demonstrably stronger evaluations. These days the standard goto method to counteract HCE's limitations is NNUE.
]

== NNUE ( ƎUИИ Efficiently Updatable Neural Networks )
#par(first-line-indent: 0em)[
  The Efficiently Updatable Neural Network (NNUE) represents a major shift in how modern game engines particularly in chess and shogi,approach evaluation functions. Originally proposed by Yu Nasu (2018) for computer shogi, NNUE introduced a hybrid paradigm that merges the pattern recognition strength of neural networks with the speed and deterministic precision of handcrafted linear evaluators. The conceptual shift towards neural networks began in specialized fields like Shogi (Sankoma-Kankei models) @Nasu2018NNUE, accelerating dramatically with the successes of AlphaZero (2017). The subsequent creation of the NNUE architecture (2018-2019) enabled top alpha-beta engines like Stockfish to successfully migrate to neural network evaluation without sacrificing brute-force speed, representing the current state-of-the-art combination @Stockfish2025NNUEWiki. The architecture was first integrated into Stockfish in 2019, a performance leap unprecedented since the rise of alpha-beta-based engines.
]
...
=== The Divergence in Neural Network Architectures
#par(first-line-indent: 0em)[
  Neural network evaluation diverges based on whether it is integrated into a traditional CPU-bound search or a selective MCTS system. NNUE is optimized for low-latency CPU inference within alpha-beta's tight performance loop @Stockfish2025NNUEWiki @Nasu2018NNUE. AlphaZero's CNNs are better suited for GPU/TPU acceleration and batch processing typical of MCTS systems @mastering. This architectural divergence reflects fundamentally different constraints: NNUE must provide evaluation fast enough for millions of positions per second in alpha-beta search, while AlphaZero is not bound by this constraint, allowing for deeper, computationally heavier neural networks that guide a selective MCTS search.
]

#par(first-line-indent: 0em)[
  The divergence is fundamentally about algorithmic goals. Traditional alpha-beta engines seek brute-force speed through guaranteed pruning. Neural network-guided MCTS engines seek efficiency through highly accurate selectivity learned from self-play @mastering[p.3-p.5]. While older MCTS implementations proved weaker than alpha-beta @mastering[p.12], coupling MCTS with deep neural networks achieved superiority, challenging the widespread belief that alpha-beta was inherently better suited for these domains. In head-to-head competition, using 64 threads and a hash size of 1GB, AlphaZero convincingly defeated all opponents, losing zero games to Stockfish @mastering[p.5].
]

=== Convergence Rather Than Replacement
#par(first-line-indent: 0em)[
  However, the subsequent integration of NNUE into Stockfish suggests a convergence rather than replacement paradigm. Modern engines like Stockfish represent a hybrid approach combining alpha-beta's efficiency with neural evaluation, rather than abandoning traditional search entirely @Stockfish2025NNUEWiki. A core research gap remains in definitively quantifying the performance comparison between these evolved approaches: highly optimized alpha-beta engines utilizing modern neural network-based evaluations (like Stockfish/NNUE) versus pure neural network-guided MCTS systems, especially under varying time controls and hardware constraints. The literature lacks comprehensive analysis of which paradigm proves superior across different computational budgets and time management scenarios.
]
