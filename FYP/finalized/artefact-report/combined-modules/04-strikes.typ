...
#let strikes_section() = [
== Strikes Attack-Generation Crate
...
`strikes` exposes static attack resources and lazy-initialized slider tables, supporting `O(1)` amortized lookups.

=== Rays, Paths, and Lookup Pipeline
...
Path queries (`between` / `through`) support legality checks and pin-related logic for constraint based move generation. 
